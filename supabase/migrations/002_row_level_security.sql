-- ============================================================
--  Seva-Sahyog Education Platform — Row Level Security
...
-- ─── Helper: get caller's role & zone from profiles ──────────
...
-- ─── PROFILES ────────────────────────────────────────────────
-- Users can read their own profile; admins read all; coordinators read own zone profiles
create policy "profiles_select" on public.profiles for select using (
  id = auth.uid()
  or public.my_role() = 'admin'
  or (public.my_role() = 'coordinator' and zone = public.my_zone())
);
...
-- Only service role / admin can insert profiles (trigger handles it)
...
create policy "centres_insert" on public.centres for insert with check (
  public.my_role() in ('admin', 'coordinator')
);
create policy "centres_update" on public.centres for update using (
  public.my_role() = 'admin'
  or (public.my_role() = 'coordinator' and zone = public.my_zone())
);
...
-- ─── STUDENTS ────────────────────────────────────────────────
-- Teachers see only their students; coordinators see their zone; admin sees all
...
create policy "students_insert" on public.students for insert with check (
  public.my_role() in ('admin', 'coordinator', 'teacher')
);
...
create policy "attendance_select" on public.attendance for select using (
  ...
  or exists (
    ...
    where s.id = student_id and s.zone = public.my_zone()
...
create policy "leaves_update" on public.leaves for update using (
  user_id = auth.uid()
  or public.my_role() in ('admin', 'coordinator')
);

-- ─── DIARY ENTRIES ───────────────────────────────────────────
create policy "diary_select" on public.diary_entries for select using (
  teacher_id = auth.uid()
  or public.my_role() = 'admin'
  or (public.my_role() = 'coordinator' and zone = public.my_zone())
);

create policy "diary_insert" on public.diary_entries for insert with check (
  teacher_id = auth.uid()
);

create policy "diary_update" on public.diary_entries for update using (
  teacher_id = auth.uid()
);

create policy "diary_delete" on public.diary_entries for delete using (
  teacher_id = auth.uid() or public.my_role() = 'admin'
);
...
create policy "announcements_delete" on public.announcements for delete using (
  author_id = auth.uid() or public.my_role() = 'admin'
);
